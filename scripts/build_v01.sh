...
# Performance renderer: start from the exact Aurora revision used by the working
# Melee handheld fast path, then re-apply Strikers' own documented Aurora fixes.
# Keeping the patch source in the Strikers tree makes conflicts explicit when either side changes.
...
git -C "$FAST_AURORA" fetch origin "$FAST_AURORA_REV" --depth 1
git -C "$FAST_AURORA" checkout -f "$FAST_AURORA_REV"
git -C "$FAST_AURORA" clean -fdx

rm -rf "$STRIKERS/extern/aurora"
cp -a "$FAST_AURORA" "$STRIKERS/extern/aurora"

# The patch was generated by the Strikers project against Aurora and contains the
# title-specific matrix, lighting, VI, CARD and capture fixes. Three-way application
# lets git use the Aurora history when the fast branch has moved nearby code.
if ! git -C "$STRIKERS/extern/aurora" apply --3way --whitespace=nowarn "$STRIKERS/tools/aurora-local-changes.patch"; then
  echo "::error::Strikers Aurora local changes did not merge cleanly onto fast Aurora"
